...
import { createProduct } from '@/app/(shop)/admin/dashboard-products/actions';

import { Category, FormInputsProduct, Product } from "@/interface";
import Image from 'next/image';
import { SizeInput, StockInput } from '@/components/ui';
...
export const DialogAdminProduct = ({ categories, product, titleButton, dialogTitle, dialogDescription, category }: Props) => {
    ...
    const { register, handleSubmit, formState: { errors }, watch, getValues, setValue } = useForm<FormInputsProduct>({
        defaultValues: {
            ...product,
            sizes: product?.sizes ?? [],
            img: undefined,
    ...
    const onSubmit = (data: FormInputsProduct) => {
    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger className="">
                {titleButton === "Create product" ? <Button className="hover:opacity-90 sm:p-[21px]" style={{backgroundColor: 'black', color: "white", borderRadius: "8px", cursor: "pointer" }}><PlusIcon /></Button> : <Button className="hover:opacity-90" style={{ padding: "21px", backgroundColor: 'black', color: "white", borderRadius: "8px", cursor: "pointer" }}><UpdateIcon /></Button>}
            </Dialog.Trigger>

            <Dialog.Content className='w-[230px] sm:w-full'  onSubmit={handleSubmit(onSubmit)}>
                ...
                <form className=''>
                    <Flex className='w-[230px] sm:w-full' direction="column" gap="1">
